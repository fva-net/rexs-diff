...
def f_c(components: list, components_prime: list):
    ...
    for c in range(len_comp):
        for c_prime in range(len_comp_prime):
            # Check if the components are of the same type and if they are, calculate the proportion of equal attributes. There are exceptions/special cases for the bearing types and the coupling types. They may be matched with each other. 
